...
const EXPIBRTDB3 = ({
  ...
  useEffect(() => {
    if (!forceExpanded) {
      ...
      loadSwitchState(
        "Exp-IB-RTDB-3-topSwitchClosed",
        setTopSwitchClosed,
        initialTopSwitchClosed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch1Closed",
        setSwitch1Closed,
        initialSwitch1Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch2Closed",
        setSwitch2Closed,
        initialSwitch2Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch3Closed",
        setSwitch3Closed,
        initialSwitch3Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch4Closed",
        setSwitch4Closed,
        initialSwitch4Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch5Closed",
        setSwitch5Closed,
        initialSwitch5Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch6Closed",
        setSwitch6Closed,
        initialSwitch6Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-3-switch7Closed",
        setSwitch7Closed,
        initialSwitch7Closed
      );
      // Only load isZoomed from localStorage if not in forceExpanded mode
      if (!forceExpanded) {
        loadSwitchState("Exp-IB-RTDB-3-isZoomed", setIsZoomed, forceExpanded);
      }
    }
  }, [
    forceExpanded,
  ...
  // dragging disabled for Exp-IB-RTDB-3
  ...
  useEffect(() => {
    ...
    if (!forceExpanded) {
      ...
      localStorage.setItem("Exp-IB-RTDB-3-isZoomed", JSON.stringify(isZoomed));
    }
  }, [
    topSwitchClosed,
    switch1Closed,
