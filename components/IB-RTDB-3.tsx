...
const IBRTDB3 = ({
  ...
  useEffect(() => {
    try {
      ...
      load("IB-RTDB-3-topSwitchClosed", setTopSwitchClosed);
      load("IB-RTDB-3-switch1Closed", setSwitch1Closed);
      load("IB-RTDB-3-switch2Closed", setSwitch2Closed);
      load("IB-RTDB-3-switch3Closed", setSwitch3Closed);
      load("IB-RTDB-3-switch4Closed", setSwitch4Closed);
      load("IB-RTDB-3-switch5Closed", setSwitch5Closed);
      load("IB-RTDB-3-switch6Closed", setSwitch6Closed);
      load("IB-RTDB-3-switch7Closed", setSwitch7Closed);
      load("IB-RTDB-3-isZoomed", setIsZoomed);
    } catch (e) {
      // ignore
    }
  }, []);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "IB-RTDB-3-topSwitchClosed",
        JSON.stringify(topSwitchClosed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch1Closed",
        JSON.stringify(switch1Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch2Closed",
        JSON.stringify(switch2Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch3Closed",
        JSON.stringify(switch3Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch4Closed",
        JSON.stringify(switch4Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch5Closed",
        JSON.stringify(switch5Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch6Closed",
        JSON.stringify(switch6Closed)
      );
      localStorage.setItem(
        "IB-RTDB-3-switch7Closed",
        JSON.stringify(switch7Closed)
      );
      localStorage.setItem("IB-RTDB-3-isZoomed", JSON.stringify(isZoomed));
    } catch (e) {}
  }, [
    topSwitchClosed,
    switch1Closed,
  ...
  const handleBottomSwitchToggle = (switchNumber: number) => {
    ...
    switch (switchNumber) {
      ...
      case 2:
        ...
        break;
      case 7:
        setSwitch7Closed(!switch7Closed);
        break;
    }
  };
            ...
            <svg width="75" height="150" viewBox="0 0 280 350">
              ...
              <foreignObject x="187" y="69" width="30" height="100">
                <Image
                  ...
                  height={60}
                  className="w-full h-full cursor-pointer"
                  onClick={() => handleBottomSwitchToggle(7)}
                />
              </foreignObject>
