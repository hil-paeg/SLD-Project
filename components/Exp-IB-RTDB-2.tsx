...
const EXPIBRTDB2 = ({
  ...
  useEffect(() => {
    if (!forceExpanded) {
      ...
      loadSwitchState(
        "Exp-IB-RTDB-2-topSwitchClosed",
        setTopSwitchClosed,
        initialTopSwitchClosed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch1Closed",
        setSwitch1Closed,
        initialSwitch1Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch2Closed",
        setSwitch2Closed,
        initialSwitch2Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch3Closed",
        setSwitch3Closed,
        initialSwitch3Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch4Closed",
        setSwitch4Closed,
        initialSwitch4Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch5Closed",
        setSwitch5Closed,
        initialSwitch5Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch6Closed",
        setSwitch6Closed,
        initialSwitch6Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch7Closed",
        setSwitch7Closed,
        initialSwitch7Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch8Closed",
        setSwitch8Closed,
        initialSwitch8Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch9Closed",
        setSwitch9Closed,
        initialSwitch9Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch10Closed",
        setSwitch10Closed,
        initialSwitch10Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch11Closed",
        setSwitch11Closed,
        initialSwitch11Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-2-switch12Closed",
        setSwitch12Closed,
        initialSwitch12Closed
      );
      // Only load isZoomed from localStorage if not in forceExpanded mode
      if (!forceExpanded) {
        loadSwitchState("Exp-IB-RTDB-2-isZoomed", setIsZoomed, forceExpanded);
      }
    }
  }, [
    forceExpanded,
  ...
  useEffect(() => {
    ...
    if (!forceExpanded) {
      ...
      localStorage.setItem("Exp-IB-RTDB-2-isZoomed", JSON.stringify(isZoomed));
    }
  }, [
    topSwitchClosed,
    switch1Closed,
  ...
  // If this component is forced expanded (used inside IB-RTDB-2 popup), notify parent.
