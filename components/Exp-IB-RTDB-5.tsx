...
const EXPIBRTDB5 = ({
  ...
  useEffect(() => {
    if (!forceExpanded) {
      ...
      loadSwitchState(
        "Exp-IB-RTDB-5-topSwitchClosed",
        setTopSwitchClosed,
        initialTopSwitchClosed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch1Closed",
        setSwitch1Closed,
        initialSwitch1Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch2Closed",
        setSwitch2Closed,
        initialSwitch2Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch3Closed",
        setSwitch3Closed,
        initialSwitch3Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch4Closed",
        setSwitch4Closed,
        initialSwitch4Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch5Closed",
        setSwitch5Closed,
        initialSwitch5Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch6Closed",
        setSwitch6Closed,
        initialSwitch6Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch7Closed",
        setSwitch7Closed,
        initialSwitch7Closed
      );
      loadSwitchState(
        "Exp-IB-RTDB-5-switch8Closed",
        setSwitch8Closed,
        initialSwitch8Closed
      );
      // Only load isZoomed from localStorage if not in forceExpanded mode
      if (!forceExpanded) {
        loadSwitchState("Exp-IB-RTDB-5-isZoomed", setIsZoomed, forceExpanded);
      }
    }
  }, [
    forceExpanded,
  ...
  // dragging disabled for Exp-IB-RTDB-5
  ...
  useEffect(() => {
    ...
    if (!forceExpanded) {
      ...
      localStorage.setItem("Exp-IB-RTDB-5-isZoomed", JSON.stringify(isZoomed));
    }
  }, [
    topSwitchClosed,
    switch1Closed,
    ...
    >
      ...
      >
        <div className="relative">
          ...
          <svg width="800" height="400" viewBox="300 -90 350 500">
